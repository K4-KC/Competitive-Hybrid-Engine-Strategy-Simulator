...
func _ready():
	# Load the spritesheet atlas before anything that needs textures
...
func select_piece(pos: Vector2i):
	...
	for target in valid_moves:
		...
		if is_capture: spawn_highlight(get_slice_texture("capture"), target)
		else: spawn_highlight(get_slice_texture("move"), target)
